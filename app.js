...
Filter Array and push array 

Solve Exercise 5 here:
*/
const highHPPokemon = pokemon.filter(pokemon => pokemon.HP > 100);
...
function evolveStarter(party, pokemon) {
    const starterIndex = party.findIndex(p => p.starter === true);
    ...
    const evolution = pokemon.find(p => p.number === starter.number + 1);
  
    if (!evolution) {
      console.log(`${starter.name} cannot evolve further.`);
      return;
    }
    party.splice(starterIndex, 1, evolution);
    console.log(`${starter.name} evolved into ${evolution.name}!`);
  }
